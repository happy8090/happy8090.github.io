<%@ Page Language="C#" MasterPageFile="~/admin/main.Master" AutoEventWireup="true" CodeBehind="OrderSet.aspx.cs" Inherits="WangQi.ERP.Web.admin.OrderSet" Title="无标题页" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MidContent" Runat="Server">
  <div id="mainMenu">
   ...
   <div id="main">
    ...
    <asp:GridView ID="DeliveryList" runat="server" AllowPaging="True" AutoGenerateColumns="False" OnPageIndexChanging="DeliveryList_PageIndexChanging" DataKeyNames="DeliveryId" PageSize="11" OnRowDeleting="DeliveryList_RowDeleting" CellPadding="0" CssClass="gv" Width="100%" EmptyDataText="暂无任何运送方式" >
        <Columns>
            <asp:BoundField DataField="DeliveryId" HeaderText="ID" >
                <ControlStyle CssClass="gvOrderId" />
            </asp:BoundField>
            <asp:BoundField DataField="DeliveryName" HeaderText="运送方式名称" ReadOnly="True" />
            <asp:BoundField DataField="DeliveryMoney" HeaderText="运送价格" ReadOnly="True" />
            <asp:TemplateField HeaderText="当前是否可用">
                <EditItemTemplate>
                    <asp:TextBox ID="TextBox1" runat="server" Text='<%#DataBinder.Eval(Container.DataItem,"DeliveryIsShow").ToString()=="False"?"否":"是" %>'></asp:TextBox>
                </EditItemTemplate>
                <ItemTemplate>
                    <asp:Label ID="Label1" runat="server" Text='<%#DataBinder.Eval(Container.DataItem,"DeliveryIsShow").ToString()=="False"?"否":"是" %>'></asp:Label>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:HyperLinkField DataNavigateUrlFields="DeliveryId" DataNavigateUrlFormatString="DeliveryEdit.aspx?deliveryId={0}"
                HeaderText="修改运送" Text="修改" >

<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PicModule.ascx.cs" Inherits="WangQi.ERP.Web.Controls.PicModule" %>
    ...
    <ItemTemplate>
        <div class="mframe">
            ...
            <div class="pack">
                ...
                <div class="mr"></div>
                <div class="mm">
                    <asp:DataList ID="ddlCategory" runat="server" CssClass="picList" EnableViewState="False" RepeatDirection="Horizontal" Width="100%">
                        <ItemTemplate>
                            <div class="picImg"> <a href ='<%# WangQi.ERP.WebsiteData.Website.IsURLRewriter() ? String.Format("{1}-D2-{0}-{2}"+WangQi.ERP.WebsiteData.Function.GetPostfixName(),Eval("PicId").ToString(),Eval("FileName").ToString(),WangQi.ERP.WebsiteData.Website.GetLanguage()) : String.Format("PicDetail.aspx?id={0}&fileName={1}&Language={2}",Eval("PicId").ToString(),Eval("FileName").ToString(),WangQi.ERP.WebsiteData.Website.GetLanguage()) %>'>
                                <%# DataBinder.Eval(Container.DataItem, "PicPathSmall").ToString().IndexOf("http://") > -1 ? "<img src='" + DataBinder.Eval(Container.DataItem, "PicPathSmall") + "' Width='" + new Unit(_width) + "' Height='" + new Unit(_height) + "' AlternateText='" + DataBinder.Eval(Container.DataItem, "Title") + "' />" : System.IO.File.Exists(System.Web.HttpContext.Current.Server.MapPath(DataBinder.Eval(Container.DataItem, "PicPathSmall").ToString())) ? "<img src='" + DataBinder.Eval(Container.DataItem, "PicPathSmall") + "' Width='" + new Unit(_width) + "' Height='" + new Unit(_height) + "' AlternateText='" + DataBinder.Eval(Container.DataItem, "Title") + "' />" : ""%></a> </div>
                            <div class="picText">
